...
import { getMissions, acceptMission, getUserAcceptedMissions } from '../services/db';
...
const Missions = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [availableMissions, userAccepted] = await Promise.all([
                    getMissions(),
                    currentUser ? getUserAcceptedMissions(currentUser.uid) : Promise.resolve([])
                ]);
    ...
    const handleAccept = async (mission) => {
        ...
        try {
            await acceptMission(currentUser.uid, mission);
